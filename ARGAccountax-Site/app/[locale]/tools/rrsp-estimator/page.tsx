...
import RrspEstimator from '@/components/calculators/RrspEstimator';
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return { title: t('rrspTitle') };
}

export default async function RrspEstimatorPage({ params }: Props) {
  const { locale } = await params;
  setRequestLocale(locale);
  return <RrspContent />;
}

function RrspContent() {
